...
- 存储：**JSON 文档数据库**（NoSQL 风格，零依赖）：`data/db/<集合>.json`，一个集合一个文档文件（`papers` / `submissions` / `mistakes` / `subjects` / `documents` / `settings`），内存读写 + 异步串行落盘（先写 `.tmp` 再改名，避免半截文件）。首次启动若发现旧版单文件 `data/db.json` 会自动迁移到集合文件，原文件备份为 `data/db.legacy.json`
